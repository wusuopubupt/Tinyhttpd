 ...
 * Returns: the socket */
/**********************************************************************/
int bind_and_listen(u_short *port)
{
    int sockfd = 0;
...
void echo_func(int client) {
    printf("client: %d\n", client);
    char buf[2048];
    read(client, buf, sizeof(buf)); 
    str_rev(buf, 0, strlen(buf)-1);
...
int main(int argc, char *argv[])
{
    ...
    u_short port = LISTEN_PORT;
    if(argc > 1) {
        port = atoi(argv[1]);
    }
    printf("port: %d\n", port);
    server_sock = bind_and_listen(&port);
